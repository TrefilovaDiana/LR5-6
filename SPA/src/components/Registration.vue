...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      validate()
      {
		  //убираем пробелы справа и слева от логина и пароля
		...
		//если все хорошо, то пытаемся понять, есть ли такой пользователь с таким логином в БД
		//для этого отправляем запрос, получаем список всех пользователей
        axios.get(this.endpoint)
                .then(response => {
					//когда пользователи пришли
                  if (response.status === 200)//статус 200 означает что запрос выполнен успешно и данные пришли. В других запросах к БД я не делаю этой проверки и вообще я думаю, она здесь и не нужна. ЕЕ можно убрать.
                  {
					  //сораняем список пользователей пришедших
